...
Run (macOS/Linux):
    ...
    # List exported volumes with paired segmentations
    python simulation/imaging/view_volume_napari.py --list

    # View by slug (data/processed/raw-extract-imaging/<slug>.{npy,json})
    python simulation/imaging/view_volume_napari.py --slug glioma_ucsf_P001_baseline

    # View NIfTI pair directly (before export pipeline exists)
    python simulation/imaging/view_volume_napari.py \\
        --mr data/raw/ucsf/patient001/T1.nii.gz \\
        --mask data/raw/ucsf/patient001/seg.nii.gz

    # Optional: overlay PDE-ready volume from solver prep
    python simulation/imaging/view_volume_napari.py --slug glioma_ucsf_P001_baseline --pde-input
...
def main() -> None:
    ...
    parser.add_argument("--demo", action="store_true", help="Synthetic MR + mask (no data required)")
    ...
    if args.list:
        for slug in list_slugs():
            print(slug)
        if not list_slugs():
            print("(none — run export pipeline or use --demo / --mr --mask)")
        return
    ...
    if not slug:
        ...
        if len(available) == 1:
            ...
            parser.error(
                "No paired volumes found. Use --demo, or --mr/--mask, or export to "
                "data/processed/raw-extract-imaging/"
            )
